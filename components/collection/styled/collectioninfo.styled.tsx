...
export const Description = ({ description }: { description: string }) => {
    return (
        <div className="py-1.5 text-sm text-gray-500 dark:text-gray-400">{description}</div>
    )
}
...
export const CollectionDatas = ({ totalVolume, floorPrice, creatorFee, follows }: CollectionProps) => {
    ...
    return (
      <div className="mt-2 flex flex-row items-center space-x-5">
        {!isScreenSmall && (
          <>
            <CollectionData data={totalVolume} icon="ri:exchange-box-fill" text="총 거래량" />
            <CollectionData data={floorPrice} icon="game-icons:token" text="최저 거래가" />
            <CollectionData data={creatorFee} icon="heroicons-solid:receipt-tax" text="작가 로열티" />
...
const CollectionData = ({ data, text, icon }: { data: number | string, text: string, icon: string }) => {
    return (
