...
export const NFTCard = ({ token }: { token: TokenData }) => {
  ...
  const fetchMetadata = async () => {
    try {
      const ipfsUrl = token.metadata.replace('ipfs://', '');
      ...
      if (data.ipfs) {
        const ipfsUrl = data.ipfs.replace('ipfs://', '');
        const response = await fetch(`https://ipfs.io/ipfs/${ipfsUrl}`);
        if (!response.ok) {
        ...
        const updatedMetaData = { ...data, imageUrl };
  ...
  return (
    <div className="relative w-72 h-fit mb-10 mx-5 rounded-lg shadow-lg dark:bg-gray-800 overflow-hidden">
      <Link href={`/collections/${_id}/nft?id=${token.tokenId}`}>
        <div className="hover:transform hover:scale-110 transition duration-300 cursor-pointer">
        <Image src={metaData.imageUrl ? metaData.imageUrl : "https://dummyimage.com/480x480/ccc/000"} alt="nft image" width={480} height={480} className="w-full h-full object-cover" />
        </div>
      </Link>
      <div className="px-5 pt-4 pb-5">
        ...
        <span className="font-bold text-2xl text-gray-900 dark:text-white flex items-center">
          <Icon icon="cryptocurrency-color:matic" className="mr-2" />{token.price}
        </span>
          <button
