...
export const TitleContainer = () => {
  ...
  return (<TitleWrap>
    <Logo xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" className="w-10 h-10 text-white p-2 bg-red-500 rounded-full" viewBox="0 0 24 24">
...
export const Button = ({ onClick, backgroundColor, color, fontSize, children }: ButtonProps) => {
  ...
  return <button onClick={onClick} className={`${bgColor} ${txtSize} ${txtColor} ${buttonStyled}`}>{children}</button>;
};
